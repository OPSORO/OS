...
class Server(object):
    ...
    def page_preferences(self):
        if request.method == "POST":
            ...
            Preferences.set(
                "audio", "master_volume", request.form.get("volume", type=int))
    ...
    def page_files(self, appname, action):
        ...
        if request.method == "POST":
            ...
            appSpecificFolderPath = request.form.get(
                "path", type=str, default="")
            extension = request.form.get(
                "extension", type=str, default=extension)
            onlyFolders = request.form.get("onlyfolders", type=int, default=0)
            saveFileView = request.form.get(
                "savefileview", type=int, default=0)
