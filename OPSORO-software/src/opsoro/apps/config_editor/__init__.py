...
config = {"full_name": "Config_Editor",
          "icon": "fa-pencil",
          'color': '#ff517e'}
    ...
    def saveconfig():
        ...
        try:
            ...
            return {
                "status": "error",
                "message":
                "Error parsing configuration files:<br/> %s<br><br> Previous configuration was restored."
                % err_msg
            }
        else:
            return {
                "message":
                "Successfully saved and parsed configuration files.<br>Loaded %d DOFs and %d servos."
                % (len(Expression.servos), len(Expression.dofs))
            }
