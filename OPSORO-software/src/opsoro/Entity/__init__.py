...
class Entity(object):
    ...
    def set_all_dofs(self,dof_value, anim_time=-1):
        """
        set all dofs on same value

        :param float dof_value:         value to set the dof, range [-1.0,1.0]
        :param float anim_time:         time for dof to move from current position to the new position
        """
        pass
