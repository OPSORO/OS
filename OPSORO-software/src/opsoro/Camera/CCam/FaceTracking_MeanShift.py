...
# PATH_HAAR_CASCADE = os.path.dirname(__file__) + '/extra/lbpcascade_frontalface.xml'
faceCascade = cv2.CascadeClassifier(PATH_HAAR_CASCADE)
termination = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 1)
REDETECT_FACE = 5  # het aantal iteraties tussen het controleren van het meanshift window of het weldegelijk een gezicht is
...
class FaceTracking():
    ...
    def update(self, frame):
        ...
        gray = cv2.equalizeHist(hsv[:, :, 2])
        # hsv = cv2.medianBlur(hsv,5)
        # cv2.imshow('hsv eq h', cv2.cvtColor(hsv[:, :, 0], cv2.COLOR_GRAY2BGR))
        new_selection = None
        ...
        if self.state == 1:  # Mean-Shift
            ...
            elif moving_dist > 250:
                self.state = 0
                new_selection = None
                print 'reset: distance'

            elif self.face_refresh_counter.check():
                if not self.faceCheck(new_selection, gray):
                    self.state = 0
                    new_selection = None
                    print 'reset: faceCheck false'
                else:
                    print 'reset: faceCheck true'
            else:
    ...
    def calHist(self, hsv, selection):
        ...
        x, y, w, h = selection
        cv2.ellipse(mask, center=(x + w / 2, y + h / 2), axes=(w / 4, h / 4), angle=0, startAngle=0, endAngle=360,
                    color=255,
                    thickness=-1)  # een cirkel als masker om zo veel mogelijk van de achtergrond weg te filteren
    ...
    def faceDetect(self, gray):
        ...
        print "FaceDetect: {}".format(time.time()-start)
        return face

    def faceCheck(self, selection, gray):
        if selection is not None:
            start = time.time()
            x,y,w,h = selection
            x_max,y_max = gray.shape
            c = 20  # extra contour
            x,y,w,h = constrain(x-c,0,x_max-w-c),constrain(y-c,0,y_max-h-c),w,h
            x,y,w,h = x,y,constrain(w+2*c,0,x_max-x),constrain(h+2*c,0,y_max-y)

            try:
                gray_select =  gray[y:y+h,x:x+w]
                gray_select = cv2.resize(gray_select,(100,100))
            except:
                print "reeor"
            ...
            print "FaceCheck: {}".format(time.time() - start)
            return len(faces) == 1
...
if __name__ == "__main__":
    ...
    while (True):
        ...
        h[:, :, 1] = 255
        h[:, :, 2] = 255;
        cv2.imshow('h', cv2.cvtColor(h[:, :, 0], cv2.COLOR_GRAY2BGR))
