...
def setup_pages(onoapp):
	...
	@visprog2_bp.route("/save", methods=["POST"])
	@onoapp.app_api
	def save():
	...
	@visprog2_bp.route("/delete/<scriptfile>", methods=["POST"])
	@onoapp.app_api
	...
	@visprog2_bp.route("/scripts/<scriptfile>")
	@onoapp.app_view
	def scripts(scriptfile):
		return send_from_directory(get_path("scripts/"), scriptfile)

	@visprog2_bp.route("/startscript", methods=["POST"])
	@onoapp.app_api
	def startscript():
		global sh
		global script
	...
	@visprog2_bp.route("/stopscript", methods=["POST"])
	@onoapp.app_api
	def stopscript():
		global sh
